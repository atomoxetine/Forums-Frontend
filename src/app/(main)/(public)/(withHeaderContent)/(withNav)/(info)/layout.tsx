...
export default function Layout({
  ...
  return <>
    ...
    <div className="grid-categories gap-4 w-full self-start">
      {categories.map((c, i) => <Category key={i} className={c.className} title={c.title} buttons={c.buttons}></Category>)}
    </div>
    <div className="h-[570px] w-screen max-w-[800px] overflow-y-scroll overflow-x-hidden bg-base-300 rounded-lg content">
      {children}
    </div>
  </>;  
}